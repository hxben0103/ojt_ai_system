...
CREATE TABLE IF NOT EXISTS attendance (
    ...
    status VARCHAR(20) DEFAULT 'Pending', -- Pending, Approved, Rejected (CRITICAL: Only Approved counts in calculations)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- ========== COMPETENCIES ==========
-- Master table for OJT competencies with point values
...
CREATE INDEX IF NOT EXISTS idx_competencies_title ON competencies(title);

-- ========== OJT DAILY TASKS ==========
-- Students log daily tasks linked to competencies
CREATE TABLE IF NOT EXISTS ojt_daily_tasks (
    task_id SERIAL PRIMARY KEY,
    student_id INT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    date DATE NOT NULL,
    task_description TEXT NOT NULL,
    hours_worked NUMERIC(4,2),
    supervisor_id INT NULL REFERENCES users(user_id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_daily_tasks_student_id ON ojt_daily_tasks(student_id);
CREATE INDEX IF NOT EXISTS idx_daily_tasks_date ON ojt_daily_tasks(date);
CREATE INDEX IF NOT EXISTS idx_daily_tasks_status ON ojt_daily_tasks(status);
CREATE INDEX IF NOT EXISTS idx_daily_tasks_supervisor_id ON ojt_daily_tasks(supervisor_id);

-- ========== TASK COMPETENCIES ==========
-- Many-to-many mapping: tasks can be linked to multiple competencies
CREATE TABLE IF NOT EXISTS task_competencies (
    task_id INT NOT NULL REFERENCES ojt_daily_tasks(task_id) ON DELETE CASCADE,
    competency_id INT NOT NULL REFERENCES competencies(competency_id) ON DELETE CASCADE,
    PRIMARY KEY (task_id, competency_id)
);

CREATE INDEX IF NOT EXISTS idx_task_competencies_task_id ON task_competencies(task_id);
CREATE INDEX IF NOT EXISTS idx_task_competencies_competency_id ON task_competencies(competency_id);
...
BEFORE UPDATE ON ojt_daily_tasks
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- CRITICAL: Only counts attendance with status = 'Approved'
...
WHERE a.status = 'Approved'  -- CRITICAL: Only approved attendance counts
