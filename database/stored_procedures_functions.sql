...
BEGIN
    SELECT jsonb_build_object(
        ...
        'attendance_stats', (
            ...
            FROM attendance a
            WHERE a.student_id = p_student_id AND a.status = 'Approved'  -- CRITICAL: Only approved attendance
        ),
        'evaluation_stats', (
            SELECT jsonb_build_object(
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'attendance', jsonb_build_object(
            ...
            'approved_records', (SELECT COUNT(*) FROM attendance WHERE status = 'Approved'),  -- CRITICAL: Only approved
            'verified_records', (SELECT COUNT(*) FROM attendance WHERE verified = TRUE AND status = 'Approved'),
            'total_hours_logged', (SELECT COALESCE(SUM(total_hours), 0) FROM attendance WHERE status = 'Approved')  -- CRITICAL: Only approved
        ),
