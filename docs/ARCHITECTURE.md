...
#### Geofence + Location Trust + Photo Evidence
- **Geofence**: Optional work-site radius; when configured and `enforceGeofence` is true, check-in can be blocked if outside (when `blockOutsideGeofence` is true) or only flagged.
- **Trust scoring**: Mock location (Android), teleport jump (>200 km/h), low accuracy (>100 m) reduce score and are stored/flagged; `blockIfMockLocation` and `flagOnlyIfSuspicious` control blocking vs flag-only. Backend sets `verification_status` to `AUTO_VERIFIED` or `FLAGGED` (e.g. outside geofence or trust_score &lt; threshold).
- **Photo evidence**: Every attendance (time-in and time-out) can require a captured photo; optional upload to backend stores path in `checkin_photo_path` / `checkout_photo_path`. Coordinator/admin see evidence (photo thumbnail, lat/lng, inside_geofence, trust_score, flags, verification_status) and can filter by `FLAGGED`.
...
- `AttendanceService`: Attendance operations (including optional geofence/trust payload)
- `EvaluationService`: Evaluation CRUD operations
- `OjtService`: OJT record management
- `OjtSitesService`: Geofence site lookup by company (for attendance check-in)
- `LocationService`: Current position via geolocator; permissions
- `GeofenceService`: Distance and inside/outside check for a site
- `LocationSecurityService`: Trust scoring (mock location, teleport, accuracy); persists last position
...
- Optional geofence/trust fields stored when provided (checkin_lat/lng, accuracy_m, distance_m, inside_geofence, trust_score, trust_flags)

#### `/api/ojt-sites` (ojtSites.js)
- CRUD for geofence sites (name, lat/lng, radius, company_id/company_name)
- Used by the app to enforce or flag check-in location
...
#### `attendance`
Daily attendance records with time-in/time-out, total hours, verification status, and images. Optional columns for geofencing: checkin_lat, checkin_lng, accuracy_m, distance_m, inside_geofence, trust_score, trust_flags.

#### `ojt_sites`
Geofence sites (name, latitude, longitude, radius_meters, company_id, company_name) for location-based attendance validation.
