...
class _SupervisorDashboardState extends State<SupervisorDashboard> {
  ...
  Widget _buildEvaluationInsight(SupervisorProvider provider) {
    ...
    return InsightCard(
      ...
      subtitle: "Review and feedback monitoring",
      icon: Icons.fact_check_rounded,
      statusLabel: allDone ? "ALL COMPLETED" : "${provider.pendingEvaluations ?? 0} Pending",
      statusColor: allDone ? AppTheme.successColor : AppTheme.warningColor,
      progressValue: provider.totalAssigned > 0 ? (provider.totalAssigned - provider.pendingEvaluations) / provider.totalAssigned : 1.0,
      insights: [
      ...
      recommendation: provider.pendingEvaluations > 0 
          ? "Complete pending evaluations to finalize student grades." 
          : "All assigned students have been evaluated for this term.",
    );
  }
  ...
  Widget _buildStudentTable(SupervisorProvider provider) {
    return ListView.separated(
      ...
      itemCount: provider.assignedStudents.length,
      separatorBuilder: (context, index) => const Divider(height: 1, indent: 64),
      itemBuilder: (context, index) {
        final record = provider.assignedStudents[index];
        ...
        return ListTile(
          ...
          subtitle: Row(
            children: [
              Text("ID: ${record.studentId ?? 'Unavailable'}", style: AppTheme.bodySmall),
              if (todayRecord != null) ...[
                const SizedBox(width: 8),
                _buildFlagIndicator(todayRecord),
              ],
            ],
          ),
